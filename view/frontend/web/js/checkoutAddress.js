        ...
        $(document).on("focusout",'input[name="postcode"]', function() {
            ...
            $.getJSON("https://brasilapi.com.br/api/cep/v1/"+ cep, function(data) {
                ...
                if(data.street != undefined){
                    ...
                    street0.css('pointer-events','none');
                    street0.attr("tabindex", "-1");
                    street0.val(rua).trigger('keyup');
                }
                if(data.neighborhood != undefined){
                    var street2 = jQuery("[name='street[2]']");

                    street2.css('pointer-events','none');
                    street2.attr("tabindex", "-1");
                    street2.val(bairro).trigger('keyup');
                }
                ...
                state_form.attr("tabindex", "-1");

                jQuery("[name='region_id'] option:not(:selected)").attr('disabled', 'disabled');
                switch(uf){
                    case "AC":
